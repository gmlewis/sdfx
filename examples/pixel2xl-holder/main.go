...
func main() {
	box := Box3D(V3{X: phoneWidth + 2*(tol+wall), Y: phoneDepth + 2*(tol+wall), Z: holderHeight + tol + wall}, round)
	cutout1 := Box3D(V3{X: phoneWidth + 2*tol, Y: phoneDepth + 2*tol, Z: holderHeight + tol + wall}, round)
	cutout1 = Transform3D(cutout1, Translate3d(V3{X: 0, Y: 0, Z: tol + wall}))
	cutout2 := Box3D(V3{X: phoneWidth + 2*tol, Y: phoneDepth + 2*(tol+wall), Z: holderHeight + tol + wall}, round)
	...
	trumpet := Extrude3D(Polygon2D(outline), phoneGrab)
	...
	wallExtrude := Extrude3D(Polygon2D(wallOutline), wall)
	...
	bottomCutoutExtrude := Extrude3D(Polygon2D(bottomCutout), 40)
	...
	RenderSTL(box, *sampling, *filename)
	fmt.Println("Done.")
}
